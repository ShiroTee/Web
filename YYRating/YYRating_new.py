...
def get_data(start, end, issuer, gap, st):
    ...
    try:
        ...
        try:
            ...
            # logging.info('Total num of data is {}'.format(total))
            # time.sleep(5)
            if total <= 300:
                ...
                if total > 100 and total <= 200:
                    ...
                    if df1.shape[0] == total:
                        ...
                        logging.info('From {} to {} Issuer {}. Not match!'.format(start, end, issuer))
                ...
                main_process(start, end, [[issuer]], gap, st)
                df1 = pd.DataFrame()
                return df1
...
def main_process(start_date, end_date, issuer_list, gap, st):
    # 获取游标
    ...
    for i in range(len(date_list) - 1):
        ...
        for j in range(len(issuer_list)):
            ...
            try:
                ...
                time.sleep(st)
                issuer_data = get_data(start, end, current_issuer, gap, st)  # 得到当前发行人的dataframe
    ...
    while status:
        ...
        if cursor.fetchall()[0][0] == 0:
            ...
            for i in error_issuer_db:
                if i[5] == 0:
                    #logging.info('+2')
                    sSql6 = '''update [riskgroup].[IA].[bond_trans_YYRating_error] set 是否删除 = 1 where 
                            开始日期 = '%s' and 结束日期 = '%s' and 发行人 = '%s' and 间隔 = '%d' and 是否错误 = '%d' '''
                    cursor.execute(sSql6 % (i[0], i[1], i[2], i[3], 1))
                    connect.commit()
                    
                    main_process(i[0], i[1], [[i[2]]], i[3], st)

                    sSql7 = '''update [riskgroup].[IA].[bond_trans_YYRating_error] set 是否错误 = 0 where 
                            开始日期 = '%s' and 结束日期 = '%s' and 发行人 = '%s' and 间隔 = '%d' and 是否删除 = '%d' '''
                    cursor.execute(sSql7 % (i[0], i[1], i[2], i[3], 1))
                    connect.commit()
                    st += 1.5
...
if __name__ == "__main__":
    ...
    sql = '''select distinct issuer from riskgroup.IA.bond_issuer_YYRating '''
    issuer_pd = pd.read_sql(sql, connect)
    issuer_list = issuer_pd.values.tolist()
    #issuer_list = [['大同煤矿集团有限责任公司'],
                   #['交通银行股份有限公司'],
                   #['上海浦东发展银行股份有限公司'],
                   #['中国民生银行股份有限公司'],
                   #['中国农业银行股份有限公司'],
                   #['中信银行股份有限公司']]
    #issuer_list = [['交通银行股份有限公司']]
    ...
    # cmd输入起止日期, 间隔
    #main_process(args.start, args.end, issuer_list, args.gap, st)

    # 输入起止日期, 间隔
    main_process('2020-01-27', '2020-07-27', issuer_list, 30, st)
    logging.info('End of spiders!')

    # 处理错误
    logging.info('Now begin exception process!')
    exception_process(st)
